<ul class="ghStarred">
  {{ $url := .Get "url" }}
  {{ range getJSON $url }}
    {{ $p := . }}
    <li class="list-group-item" style="list-style: none;" class="list-group">
      <a href="{{$p.owner.html_url}}" target="_blank">
        <img src="{{$p.owner.avatar_url}}" alt="Avatar of {{$p.owner.login}}" title="Avatar of {{$p.owner.login}}" style="height:100px;" >
      </a>
      {{$p.language}}: <strong>{{ $p.name }}</strong>
      <a href="{{ $p.html_url }}" target="_blank">{{ $p.full_name }}</a>
      <br>
      Stars: {{$p.stargazers_count}} | Forks: {{$p.forks_count}} | Size: {{$p.size}}<br>
      {{ $p.description }}
      <div class="clear"></div>
    </li>
    {{ end }}
</ul>
